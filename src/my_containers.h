...
class vector {
  ...
  T& operator[] (int i) {
    assert(i >= 0 && i < size());// << "Bad index: " << i << " (vector size " << size() << ")";
    return impl[i];
  }
  ...
  vector<T> getSubsequence(int start, optional<int> lengthOption = none) const {
    ...
    assert(start >= 0 && length >= 0 && start + length <= size());
    vector<T> ret;
    ret.reserve(length);
  ...
  class Iterator : public std::iterator<std::random_access_iterator_tag, T> {
    ...
    void checkParent() const {
      assert(parent->modCounter == currentMod);// << "Container was modified during iteration.";
    }
