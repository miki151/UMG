...
class PrettyInputArchive {
  public:
    PrettyInputArchive(const vector<string>& inputs, const vector<string>& filenames) {
      string allInput;
    ...
    void error(const string& s) {
      ...
      if (auto& filename = pos.filename)
        msg = *filename + ": ";
      throw PrettyException{msg + "line: " + to_string(pos.line) + " column: " + to_string(pos.column) + ": " + s};
    }
      ...
      if (defs.count(name)) {
        ...
        if (argLocs.size() != info.params.size())
          error("Wrong number of arguments to macro " + name + ". Expected " +
              to_string(info.params.size()) + ", got " + to_string(argLocs.size()));
        auto saveDefs = defs;
...
template <class T>
inline void serialize(PrettyInputArchive& ar1, NameValuePair<T>& t) {
  handleNamePair(ar1, t.name, t.value, false);
}

template <class T>
inline void serialize(PrettyInputArchive& ar1, NameValuePair<optional<T>&>& t) {
  handleNamePair(ar1, t.name, t.value, true);
}
...
// Ignore these inputs
template <class T> inline
void prologue(PrettyInputArchive&, NameValuePair<T> const & ) { }

template <class T> inline
void epilogue(PrettyInputArchive&, NameValuePair<T> const & ) { }
